(function() {
	...
	const texts = {
		...
		countrySearch: root.dataset.textCountrySearch || 'Search country or code',
	...
	function getCountryName(option) {
		if (option.name) {
			return option.name;
		}
		try {
			return regionNames ? regionNames.of(option.flag.toUpperCase()) || option.code : option.code;
		} catch (e) {
			return option.code;
		}
	}
	...
	function applyCountrySelection(option) {
		...
		els.countryToggle.innerHTML = '' +
			'<span class="twofactor-kannel-setup__country-value">' +
				'<span class="fi fi-' + option.flag + '"></span>' +
				'<span>' + option.label + '</span>' +
			'</span>' +
			'<span class="twofactor-kannel-setup__caret" aria-hidden="true"></span>';
	...
	function renderCountryOptions(filterValue) {
		...
		COUNTRY_OPTIONS.filter(function(option) {
			if (!filter) {
				return true;
			}
			const haystack = [option.code, option.label, getCountryName(option)].join(' ').toLowerCase();
			...
			item.role = 'option';
			item.innerHTML = '' +
				'<span class="twofactor-kannel-setup__dropdown-main">' +
					'<span class="fi fi-' + option.flag + '"></span>' +
					'<span class="twofactor-kannel-setup__dropdown-code">' + option.label + '</span>' +
				'</span>' +
				'<span class="twofactor-kannel-setup__dropdown-name">' + getCountryName(option) + '</span>';
	...
	function renderCountryDropdown() {
		...
		searchWrap.className = 'twofactor-kannel-setup__dropdown-search';
		countrySearchInput = document.createElement('input');
		countrySearchInput.type = 'text';
		countrySearchInput.className = 'twofactor-kannel-setup__dropdown-search-input';
		countrySearchInput.placeholder = texts.countrySearch;
		countrySearchInput.addEventListener('input', function() {
			renderCountryOptions(countrySearchInput.value);
		});
		searchWrap.appendChild(countrySearchInput);
		countryOptionsList = document.createElement('div');
		countryOptionsList.className = 'twofactor-kannel-setup__dropdown-list';
		els.countryDropdown.appendChild(searchWrap);
		els.countryDropdown.appendChild(countryOptionsList);
		renderCountryOptions('');
	}
	...
	els.countryToggle.addEventListener('click', function() {
		...
		if (!expanded && countrySearchInput) {
